...
def linearRegresssion(x, y, w, alpha):
    """
    x,y: numpy array
    ...
    derivative = 1
    i = 0
    while(abs(derivative) > 0.01):
        # print w and loss, iteration
        i += 1
        derivative = np.sum(x*(w*x-y))/len(x)
        loss = np.sum((w*x-y)**2)/(2*len(x))
        # print(w, loss, i)
...
def linearRegresssionAffine(x, y, w, alpha, b):
    ...
    while((abs(derivative) > 0.01) and (abs(der_b) > 0.01)):
        # print w and loss, iteration
        i += 1
        derivative = np.sum(x*(w*x+b-y))/len(x)
        loss = np.sum((w*x+b-y)**2)/(2*len(x))
        der_b = np.sum(w*x+b-y)/len(x)
        print(w, b, loss, i)
        w = w - alpha*derivative
        b = b - alpha*der_b
...
slope = linearRegresssion(x, y, 1, 0.1)
print(linearRegresssion(x, y, 1, 0.1))
...
new_slope, b,i = linearRegresssionAffine(x, y, 1, 0.05, 1)
print("new_slope, b, slope", new_slope, b,slope)

#visualize
plt.plot(x, y, 'bo',label='GDP vs Elec Consumption')
plt.plot(x, new_slope*x+b, '-',label='Affine Function Regression')
plt.plot(x, slope*x, ':',label='Linear Regression')
plt.ylabel('Electricity consumption(kwh per capita')
plt.xlabel('GDP')
